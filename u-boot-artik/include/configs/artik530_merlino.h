...
#define CONFIG_DFU_ALT \
	...
	"/s5p4418-artik532-merlino-rev03.dtb ext4 $rootdev $bootpart;" \
	"/s5p4418-artik530-merlino-rev03.dtb ext4 $rootdev $bootpart;" \
	"/s5p4418-artik530-merlino-rev00.dtb ext4 $rootdev $bootpart;" \
...
#define CONFIG_EXTRA_ENV_SETTINGS					\
	...
	"                number=$loop\n"				\
	"            fi\n"						\
	"            ext4size mmc $rootdev:$bootpart s5p4418-artik${model_id}-merlino-rev${number}.dtb && setexpr success 1\n" \
	"            setexpr loop $loop - 1\n"				\
	"        done\n"						\
	"        if test $success -eq 0; then\n"			\
	"            ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-artik530-merlino-rev00.dtb\n" \
	"        else\n"						\
	"            ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-artik${model_id}-merlino-rev${number}.dtb\n" \
	"        fi\n"							\
	"    else\n"							\
	...
	"    setenv bootargs ${console} "				\
		...
		"drm_panel=$lcd_panel nr_cpus=${nr_cpus} ${loglevel} "	\
		"${opts} bootfrom=${bootpart} rescue=${rescue}\0"	\
	"load_kernel=\n"						\
	"    ret=0\n"							\
